...
--- DEBUG NOTE ---
Is file me kuch log.info(...) lines temporarily add ki gayi hain (sirf logging,
koi behavior/feature change nahi) taaki Render logs me exactly dikhe ki /start
ka flow kahan tak pahuncha aur kahan ruka. Ye lines baad me hata sakte ho.
"""
...
async def send_welcome(chat_id, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"[send_welcome] START chat_id={chat_id}")

    settings = db.get_settings()
    logger.info(f"[send_welcome] settings fetched: exists={settings is not None}")

    markup = kb.courses_kb()
    course_count = len(db.list_courses())
    active_course_count = len([c for c in db.list_courses() if c.get("active", True)])
    logger.info(f"[send_welcome] total_courses={course_count} active_courses={active_course_count} markup_is_none={markup is None}")

    if not markup:
        logger.info("[send_welcome] NO ACTIVE COURSES -> sending fallback text and returning")
    ...
    logger.info(f"[send_welcome] media_type={media_type} has_file_id={bool(file_id)}")
    ...
    try:
        if media_type == "photo" and file_id:
            source = file_id
            if not is_master:
                # Clone/Buyer bot Master bot ka file_id seedha use nahi kar sakta
                # (Telegram file_id sirf usi bot ke liye valid hota hai) --
                # isliye Master bot se file download karke fresh bhejte hain.
                logger.info("[send_welcome] non-master bot -> refetching photo via Master bot")
                fetched = await utils.refetch_file_bytes(file_id)
                source = fetched if fetched is not None else None
            if source is not None:
                logger.info("[send_welcome] sending PHOTO welcome")
                result = await utils.safe_send_photo(context.bot, chat_id, source, caption, reply_markup=markup)
            else:
                logger.info("[send_welcome] photo refetch failed -> falling back to TEXT welcome")
                result = await utils.safe_send_message(context.bot, chat_id, caption, reply_markup=markup)
        elif media_type == "video" and file_id:
            source = file_id
            if not is_master:
                logger.info("[send_welcome] non-master bot -> refetching video via Master bot")
                fetched = await utils.refetch_file_bytes(file_id)
                source = fetched if fetched is not None else None
            if source is not None:
                logger.info("[send_welcome] sending VIDEO welcome")
                result = await utils.safe_send_video(context.bot, chat_id, source, caption, reply_markup=markup)
            else:
                logger.info("[send_welcome] video refetch failed -> falling back to TEXT welcome")
                result = await utils.safe_send_message(context.bot, chat_id, caption, reply_markup=markup)
        else:
            logger.info("[send_welcome] sending TEXT welcome")
            result = await utils.safe_send_message(context.bot, chat_id, caption, reply_markup=markup)
        logger.info(f"[send_welcome] send result is None={result is None}")
    except Exception:
        logger.exception("[send_welcome] EXCEPTION while sending welcome message")
        raise

    logger.info("[send_welcome] DONE")


async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    logger.info(f"[cmd_start] START user_id={user.id} username={user.username} chat_id={update.effective_chat.id}")

    context.user_data.clear()

    try:
        db.upsert_user(user.id, user.username, user.full_name)
        logger.info("[cmd_start] upsert_user OK")
    except Exception:
        logger.exception("[cmd_start] EXCEPTION in upsert_user")
        raise

    try:
        await utils.animate_progress(context.bot, update.effective_chat.id, label="✨ Loading Courses")
        logger.info("[cmd_start] animate_progress OK")
    except Exception:
        logger.exception("[cmd_start] EXCEPTION in animate_progress")
        raise

    try:
        await send_welcome(update.effective_chat.id, context)
        logger.info("[cmd_start] send_welcome OK")
    except Exception:
        logger.exception("[cmd_start] EXCEPTION in send_welcome (from cmd_start)")
        raise

    logger.info("[cmd_start] DONE")
...
async def show_item_detail(update: Update, context: ContextTypes.DEFAULT_TYPE, item_id: str):
    ...
    if file_id and media_type in ("photo", "video"):
        source = file_id
        if not is_master:
            # Master bot ka file_id Buyer/clone bot se seedha use nahi hota (400 Bad Request) --
            # Master bot se file download karke fresh bhejte hain.
            fetched = await utils.refetch_file_bytes(file_id)
            source = fetched if fetched is not None else None
        if source is not None:
            if media_type == "photo":
                await utils.safe_send_photo(context.bot, q.message.chat_id, source, caption, reply_markup=markup)
            else:
                await utils.safe_send_video(context.bot, q.message.chat_id, source, caption, reply_markup=markup)
            return
