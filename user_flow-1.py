...
uske baad woh MongoDB me cache ho jaata hai -- agli baar se turant (bina download) chalta
...
async def _send_media(context, chat_id, source_file_id, media_type, caption, markup):
    ...
    if is_master:
        ...
        bot_key = utils.clone_path_id(bot.token)
        cached = db.get_cached_file_id(bot_key, source_file_id)
        if cached:
            source = cached
        else:
