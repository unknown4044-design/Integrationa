...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, action: str):
    ...
    # ---- Courses ----
    ...
    elif key == "togglecourse":
        course_id = parts[1]
        course = db.get_course(course_id)
        db.set_course_active(course_id, not course.get("active", True))
        course = db.get_course(course_id)
        status = "✅ Active (buyer ko dikh raha hai)" if course.get("active", True) else "⛔ Disabled (buyer ko nahi dikhega)"
        await q.edit_message_text(
            f"📚 <b>{course['name']}</b>\nStatus: {status}\nItems/Batches manage karo:",
            parse_mode=ParseMode.HTML, reply_markup=kb.admin_course_manage_kb(course_id),
        )
        ...
        item_id, course_id = parts[1], parts[2]
        item = db.get_item(item_id)
        db.set_item_active(item_id, not item.get("active", True))
        item = db.get_item(item_id)
        status = "✅ Active (buyer ko dikh raha hai)" if item.get("active", True) else "⛔ Disabled (buyer ko nahi dikhega)"
        await q.edit_message_text(
            f"🎯 <b>{item['name']}</b>\nPrice: ₹{item['price']}\nStatus: {status}",
            parse_mode=ParseMode.HTML, reply_markup=kb.admin_item_manage_kb(item_id, course_id),
        )
        ...
        item_id, course_id = parts[1], parts[2]
        _set_state(context, "renameitem", item_id=item_id, course_id=course_id)
        await q.edit_message_text("Item/Batch ka naya naam type karo:", reply_markup=kb.cancel_kb())
    ...
    # ---- Groups ----
        ...
        await q.edit_message_text(
            "✅ <b>Sabse aasan tarika:</b> Bot ko us group/channel me seedha <b>Admin</b> bana do "
            "(invite-link banane ki permission ke saath) — add hote hi yeh khud-ba-khud yahan list ho jayega, "
            "kuch aur karne ki zarurat nahi.\n\n"
            "Agar kisi wajah se auto-add na ho, to fallback: group se koi message yahan forward karo, "
            "ya <code>Group Name | -1001234567890</code> format me manually bhejo.",
...
async def review_reject(update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
    ...
    if log_msg:
        caption = utils.build_order_caption(order, status_label="Rejected ❌")
        await log.edit_log_caption(context, log_msg[0], log_msg[1], caption)
...
async def review_group_selected(update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str, group_id: str):
    ...
    log_msg = order.get("log_msg")
    if log_msg:
        caption = utils.build_order_caption(order, status_label=f"Accepted ✅ → {group['name']}")
        await log.edit_log_caption(context, log_msg[0], log_msg[1], caption)
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if state == "renamecourse" and update.message.text:
        db.update_course_name(data["course_id"], update.message.text.strip())
        _clear_state(context)
