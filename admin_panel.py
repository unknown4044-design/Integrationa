...
Content (courses/items/banner/caption/price), groups, aur saare text-messages
yahin se runtime me configure hote hain -- code me kuch bhi hardcoded nahi.
"""
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, action: str):
    ...
    # ---- Orders ----
        ...
        if not pend:
            text = "Koi pending order nahi hai."
        else:
            lines = [f"🆔 <code>{o['_id']}</code> — {o['item_name']} — ₹{o['final_amount']}" for o in pend]
            text = "🕓 <b>Pending Orders</b>\n" + "\n".join(lines)
        await q.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb.admin_main_kb())
...
async def review_reject(update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
    ...
    try:
        await context.bot.send_message(order["user_id"], reject_msg)
    except Exception:
        pass
...
async def handle_admin_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Returns True agar photo/video admin ke 'set banner' state ne consume kar liya."""
    if not _guard(update):
        return False
    if context.user_data.get("adm_state") != "setbanner":
        return False
    data = context.user_data.get("adm_data", {})
