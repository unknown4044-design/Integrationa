...
# ---------------- Cross-bot file re-fetch (cached, fast) ----------------
# Telegram ka file_id sirf usi bot ke liye valid hota hai jisne file pehli baar
# upload/receive ki thi. Master bot pe admin ne jo banner set kiya, uska file_id
# Buyer (clone) bot se seedha use nahi ho sakta -- 400 Bad Request aata hai.
# Isliye PEHLI baar Master bot se download+upload karke naya file_id nikalte hain,
# phir usko cache kar dete hain -- agli baar se turant (bina download kiye) chalta hai.

async def refetch_file_bytes(file_id: str):
    """Master bot ke through file download karke bytes (BytesIO) return karta hai.
    Fail hone par None return karta hai. Sirf tab chalta hai jab cache me file na ho."""
    from telegram import Bot
    try:
        master_token = os.environ["BOT_TOKEN"]
        master_bot = Bot(master_token)
        tg_file = await master_bot.get_file(file_id)
...
async def animate_progress(bot, chat_id, label="Loading", steps=3, delay=0.12, bar_len=10):
    try:
