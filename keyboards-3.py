...
def courses_kb():
    buttons = [InlineKeyboardButton(c["name"], callback_data=f"course:{c['_id']}") for c in db.list_courses()]
    return InlineKeyboardMarkup(_grid(buttons)) if buttons else None


def items_kb(course_id):
    buttons = [InlineKeyboardButton(i["name"], callback_data=f"item:{i['_id']}") for i in db.list_items(course_id)]
    rows = _grid(buttons)
...
def admin_courses_kb():
    buttons = [InlineKeyboardButton(c["name"], callback_data=f"adm:course:{c['_id']}") for c in db.list_courses()]
    rows = _grid(buttons)
...
def admin_course_manage_kb(course_id):
    buttons = [InlineKeyboardButton(i["name"], callback_data=f"adm:item:{i['_id']}") for i in db.list_items(course_id)]
    rows = _grid(buttons)
