...
async def send_welcome(chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if media_type == "photo" and file_id:
            ...
            result = await utils.safe_send_photo(context.bot, chat_id, file_id, caption, reply_markup=markup)
        elif media_type == "video" and file_id:
            logger.info("[send_welcome] sending VIDEO welcome")
            result = await utils.safe_send_video(context.bot, chat_id, file_id, caption, reply_markup=markup)
        else:
...
async def on_screenshot_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if log_msg:
        db.set_order_log_msg(order_id, log_msg.chat_id, log_msg.message_id)    file_id = settings.get("welcome_media_file_id")
    if media_type == "photo" and file_id:
